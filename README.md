...
>
> New in v0.3.4: **PoolTerminal no longer pins a CPU core while it is open.** Three animation loops were repainting the dashboard sixty times a second, with the heartbeat's glow re-blurred on every frame, which held the web process at 100-135% CPU on every machine the app ran on. Motion now runs at 1 Hz with writes skipped when nothing changed, gauges and flashes step instead of gliding so a value that changes every second no longer keeps the page repainting every frame, and everything pauses while the window is hidden: the same display for a fraction of the CPU, which matters on a laptop and on a desktop that also runs a node. Also stops the Node Health Forge card flagging CHECK on healthy pools, and shows forge counts against how often the node was actually scheduled.
>
...
- **DELEGATORS** - One data-rich, sortable table of every delegator, merging live stake with a computed **loyalty** ranking (tenure × stake-weight × penalties for defection/withdrawal). Sort by loyalty or stake, dust filter, paginated. **Search by stake address** to jump straight to a delegator and highlight their row, and **copy** any full stake address with one click. Click any delegator for a deep-dive: balance, rewards, withdrawals, DRep flag, and a colourful **pool-movement journey** showing every pool they've delegated to with entry/exit epochs and active stake at each. Each row has two history buttons: **Delegation history** (the pool-movement journey) and **Stake history** - a per-epoch active-stake table (balance, change, running balance going back in time) paired with a running-balance line graph across the delegator's whole history. Stake history works from db-sync, Koios *or* Blockfrost; on db-sync it additionally shows a **Movements** list - the ADA actually moved in and out per transaction, alongside rewards in, withdrawals out and treasury / reserves payouts - so a balance change can be traced to the transaction behind it, with the net for the epoch and the epoch that stake becomes active in. The deep-dive works from db-sync, Koios *or* Blockfrost; the loyalty leaderboard needs db-sync or Blockfrost (Koios can't compute it). Delegator figures are reconciled against **live account state**: a delegator's per-epoch series is *active stake*, frozen at the epoch boundary, while an explorer shows the *live* balance (spendable UTxO + undrawn rewards). PoolTerminal shows both, labels which is which - the stake column names the snapshot epoch it came from - and spells the arithmetic out, so a figure that reads a little under Cardanoscan is explained rather than left looking like a bug.
...
- **NOTIFICATIONS** - A live feed of delegation activity, detected on-chain within minutes: delegators joining (with the pool they came from), transfers in, **returning** delegators (anyone who was ever delegated before), redelegations away, and stake increases/decreases. Each event is colour-coded with a from→to flow, amount, epoch, slot, UTC timestamp and a one-click Cardanoscan transaction link. Each event's stake address can be **copied** with one click (handy for pasting into the Delegators search). When a returning delegator's true prior pool can't be resolved from the active source (e.g. a same-epoch multi-hop that epoch-grained APIs can't see), the event is honestly labelled **Returning Former Delegator** with the origin shown as unknown and a hint that db-sync resolves the full transfer chain. A **Clear history** button wipes the displayed feed while leaving monitoring intact. Corner toasts surface activity from any tab, and an unread badge tracks new events. A built-in advisor scales the polling cadence to your delegator count and chosen source so notifications stay within free-tier limits. Systemic shifts that would otherwise flood the feed - the whole stake distribution moving at an epoch boundary, a data-source switch changing the stake basis, or any mass change across many delegators at once - are detected and absorbed silently, so only genuine per-delegator activity surfaces. At the epoch boundary this is done **precisely** where db-sync is available: PoolTerminal reads what each delegator was actually credited at the roll and subtracts it, so the reward bump nets to nothing while a genuine payment landing in the same window - an operator returning fees, say - still fires, labelled *excl. epoch reward*. Accounts mid-move are held rather than re-baselined, so nothing is silently swallowed.
- **ALERTS** - **Telegram alerting with nothing installed on your block producer.** Every other SPO alerting setup wants an agent, a cron job or a monitoring stack running next to your node; PoolTerminal already receives this data every poll cycle, so it just sends it on. A guided three-step setup (create a bot with @BotFather → paste the token → PoolTerminal detects your chat ID automatically) and a **Test** button on every alert so you can see exactly what each one looks like on your phone before you rely on it. Six alerts ship today: **KES expiry** (days before your key runs out), **node stall** (tip stopped advancing), **block minted** (you forged one), **slow propagation**, **mempool full** and **low peers** - each independently toggled with its own threshold. A global cooldown and optional **quiet hours** stop a flapping condition becoming a night of buzzing, and a recent-alerts list shows what fired and when. The bot token travels to Telegram as a real HTTPS header from the Rust backend, so it never appears in a command line on your node. Alerts fire while PoolTerminal is open - by design, since it is meant for an always-on desktop, and it means no agent, no extra attack surface and nothing new to keep patched on the BP.
...
| Telegram alerts (KES, stall, blocks, propagation, mempool, peers) | ✓ | | | |
...
```bash
sudo dpkg -i PoolTerminal_0.3.0_amd64.deb
```
...
- **Node credentials are never persisted.** SSH passwords, OTP codes, and SSH key passphrases live only in memory for the session - never written to disk, in any mode. This is the credential that could actually reach your node, and PoolTerminal will ask you for it again rather than keep it.
- **Some API credentials are stored locally, on purpose - and here is exactly where.** These persist because the features would otherwise stop working every time you close the app. All of them live unencrypted inside PoolTerminal's own application-data directory (`~/.local/share/com.gnp1.poolterminal/` on Linux):
...
**Working:** the full data layer (node, Koios, db-sync, Blockfrost), connection (SSH + local), the setup wizard, and the DASHBOARD / HISTORY / DELEGATORS / NOTIFICATIONS / NODE HEALTH / LOGS / ALERTS / DATA / MAP views - including per-delegator stake history reconciled against live account state, op-cert counter monitoring, honest returning-delegator labelling, propagation history with the epoch-transition view, and Telegram alerting with nothing installed on the block producer. Dedicated **Relay 1 / Relay 2** tabs work too: isolated, node-only relay dashboards with heartbeat, density, mempool, tip/sync, epoch, block propagation, a scrollable peer list, a geolocated relay map, background-persistent polling, and automatic node selection for co-located BP+relay hosts.

**Known rough edges:** the setup wizard is still being refined. Slow queries on a busy block producer (the leadership schedule especially) can take up to two minutes on first connect - that's the node, not the app, and it's signposted in the UI. Alerts fire only while PoolTerminal is open, by design. Some things have only ever been tested against a single node layout.
